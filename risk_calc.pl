...
sub parse_access_list() {
  ...
  #
	...
	if ($verbose) { 	# debugging checkpoint #2
...
sub object_group_lookup () {
  #
  ## Perform object group lookup for ACL; use object group name as input, return the network blocks, service ports as an array accordingly
  #
	...
	while (<IN3>) {
		...
		if ((/^object-group\s(service|network)\s/) && (/\s+$obj_gp_name\s+/)) {
			$recording++;
			if ($line =~ /^object-group\s(service|network)\s/gi) { 
				$obj_type=$1; 
			}
			next;
		}
		if ($recording && ($obj_type eq "network")) {
			if ($line =~ /^\s+description/) { next; }		# skip on "description xxxx" line
			$line=~s/^\s+//g;
			my @LINE=split(/\s+/,$line);
			if ($LINE[0] eq "group-object") {			# recursive function is used to process nested "group-object"
				my @B=object_group_lookup($LINE[1],$fw_conf);
				push(@BLOCKS,@B);
			} elsif ($LINE[1] eq "host") {
				my $blk;
				if ($LINE[2] =~ /\d+\.\d+\.\d+\.\d+/){
					$blk=$LINE[2]."/32";
				} else {
					my $ip = &object_name_lookup($LINE[2],$_[1]); 
					$blk=$ip."/32";
				}
